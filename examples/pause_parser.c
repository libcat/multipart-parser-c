...
int main(int argc, char **argv) {
    ...
    while ((buffer_len = read(0, buffer, sizeof(buffer))) > 0) {
        buffer_parsed = 0;
        size_t parsed;
        while ((parsed = multipart_parser_execute((multipart_parser *)&parser, &buffer[buffer_parsed], buffer_len - buffer_parsed)) > 0) {
            ...
            // since it paused,
            // you may return here when work with llhttp
            buffer_parsed += parsed;
        }

        // move remaining buffer to head
        // if invoke in llhttp, you may simply reuse the buffer
